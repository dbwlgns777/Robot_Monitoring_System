...
describe('administrator registration approval API',()=>{
 it('calls the protected approval endpoint with the session cookie',async()=>{
  ...
  await authApi.approveRegistration(7);

  expect(fetchMock).toHaveBeenCalledWith('/api/v1/admin/registration-requests/7/approve',expect.objectContaining({method:'POST',credentials:'include'}));
 });
});
